...
import {
  ITableFilter,
  ITableFilterSettings,
} from '../../shared/interfaces/table-filter.interface';
...
export class BooksComponent implements OnInit {
  ...
  public isTakeBookDisabled(book: IBook): boolean {
    ...
    if (book.availability.some((x) => x === BookAvailability.inTheRoom)) {
      countCondition =
        this.user?.books.filter((x) => !x.isTakeAway).length >= 10;
    } else {
  ...
  public onSelectFilterField(filterFieldName: string): void {
    const books = this.dataSource.data.map((x) => ({
      category: x.category,
      author: x.author,
    ...
    if (filterFieldName === BookTableFiltersValue.category) {
      ...
      this.filterValues = new Set(
        books.map((x) => x[filterFieldName as keyof {}])
      );
    }
  }
  ...
  public onApplyFilter(filterSettings: ITableFilterSettings[]): void {
    filterSettings.forEach((setting) => {
      const allBooks =
        this.dataSource.data.length && filterSettings.length > 1
          ? this.dataSource.data
          : this.books;
      if (setting.field === BookTableFiltersValue.category) {
        this.dataSource.data = allBooks.filter((x) =>
          setting.value.some((y) => x.category.includes(y))
        );
      } else if (setting.field === BookTableFiltersValue.availability) {
        this.dataSource.data = allBooks.filter((x) =>
          setting.value.some((y) => x.availability.includes(y))
        );
      } else if (setting.field === BookTableFiltersValue.author) {
        this.dataSource.data = allBooks.filter((x) =>
          setting.value.includes(x.author)
        );
      }
    });
  }
  ...
  private takeBook(book: IBook, isTakeAway: boolean): void {
    ...
    this.userBookService.takeUserBook(user._id, newBook).subscribe(
      (user) => {
        this.user = user;
  ...
  private returnBook(id: string): void {
    ...
    this.userBookService
      .returnUserBook(this.user._id, user.books)
      .pipe(untilDestroyed(this))
      .subscribe(
        (user) => {
          this.user = user;
